...
import { Plus, Hop as Home, ChartBar as BarChart3, Eye, FileText, Star, CircleCheck as CheckCircle2, Clock, CircleAlert as AlertCircle, TrendingUp } from "lucide-react";
...
function SellerDashboard() {
  const { loading, user, firstName } = useRequireRole(["seller"]);
      ...
      try {
        const [listings, requests] = await Promise.all([
          ...
          supabase
            ...
            .select("id")
            .eq("property_seller_id", user.id),
        ]);

        setStats({
          totalListings: listings.data?.length || 3,
          propertyViews: Math.floor(Math.random() * 400) + 80,
          viewingRequests: requests.data?.length || 8,
          soldProperties: Math.floor(Math.random() * 5) + 1,
        });
        ...
        setViewingRequests([
          {
            ...
            propertyTitle: "Modern 3BR Villa in Westlands",
            scheduledAt: "2025-07-25 11:00 AM",
            status: "pending",
          },
          {
            id: "req-2",
            requesterName: "Bob Wilson",
            propertyTitle: "Luxury 4BR Penthouse in Karen",
            scheduledAt: "2025-07-23 03:30 PM",
            status: "confirmed",
          },
        ]);
      } catch (error) {
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: `${stats.viewingRequests} inquiries`,
    },
    {
      to: "#",
      icon: Star,
  ...
  const filteredProperties =
    statusFilter === "all"
      ? sellerProperties
      : sellerProperties.filter((p) => p.status === statusFilter);
